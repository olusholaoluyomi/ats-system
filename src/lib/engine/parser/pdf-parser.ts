...
// Defensive polyfill: the legacy pdfjs build already ships its own internal
// shim for Promise.withResolvers(), but this covers any other code path
// (present or future) that might assume the method exists natively.
// Promise.withResolvers() only reached broad browser support in March 2024
// (Safari 17.4+); on any iPhone running an older iOS version, calling it
// directly throws "undefined is not a function" - which is exactly the
// crash iOS users were hitting here, since the *standard* pdfjs-dist build
// calls it with no fallback. See https://github.com/mozilla/pdf.js/issues/20479.
if (typeof (Promise as { withResolvers?: unknown }).withResolvers !== 'function') {
	(Promise as unknown as { withResolvers: <T>() => PromiseWithResolversResult<T> }).withResolvers =
		function <T>() {
